...
class AppointmentController extends Controller
{
    ...
    {
        $upcomingAppointmets = Appointment::whereIn('status', ['Active', 'Follow-up'])
            ->get();
        ...
        foreach ($upcomingAppointmets as $appointment) {
            ...
            if ($user->id == $request->user()->id) {
                $studentAppointments[] = [
                    'appointment' =>
                    $appointment->only('id', 'meetingType', 'description', 'created_at'),
    ...
    {
        $upcomingAppointmets = Appointment::whereIn('status', ['Done', 'Cancelled'])
            ->get();
